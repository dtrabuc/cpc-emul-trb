# backend/core/keyboard_state.py
# Matrice de clavier du CPC 464 – 10 lignes × 8 colonnes
# Toutes les touches de A à Z, chiffres, symboles, et touches spéciales sont définies
...
class KeyboardState:
    # ============================================================
    # 1. DÉFINITION DE TOUTES LES TOUCHES (ordre ligne par ligne)
    # ============================================================
    ...
    # --- Ligne 1 : F9, puis chiffres 1 à 7 ---
    F9 = CPCKey(1, 7)
    NUM1 = CPCKey(1, 0)
    NUM2 = CPCKey(1, 1)
    NUM3 = CPCKey(1, 2)
    ...
    # --- Ligne 2 : chiffres 8,9,0, puis A,Z,E,R,T ---
    NUM8 = CPCKey(2, 0)
    NUM9 = CPCKey(2, 1)
    NUM0 = CPCKey(2, 2)
    A = CPCKey(2, 3)
    ...
    # --- Ligne 7 : Esc, Tab, Caps Lock, Shift (gauche/droite), Ctrl ---
    ESC = CPCKey(7, 0)
    TAB = CPCKey(7, 1)
    ...
    # ============================================================
    # 2. DICTIONNAIRE AZERTY → CPCKey (TOUTES les touches existent)
    # ============================================================
    AZERTY_MAP = {
        # Lettres minuscules (AZERTY)
    ...
    # Matrice 10 lignes × 8 colonnes (toutes les touches = 1 = relâchée)
    _matrix = [0xFF] * 10

    @classmethod
    def press_key(cls, key: CPCKey):
        """Appuie sur une touche (met le bit à 0)"""
        cls._matrix[key.line] &= ~(1 << key.bit_pos)

    @classmethod
    def release_key(cls, key: CPCKey):
        """Relâche une touche (met le bit à 1)"""
        cls._matrix[key.line] |= (1 << key.bit_pos)

    @classmethod
    def read_row(cls, row: int) -> int:
        """Lit une ligne de la matrice (0 = appuyé, 1 = relâché)"""
        return cls._matrix[row] if 0 <= row < 10 else 0xFF

    @classmethod
    def press_azerty(cls, char: str) -> bool:
        """Appuie sur une touche à partir d'un caractère AZERTY"""
    ...
    def release_azerty(cls, char: str) -> bool:
        """Relâche une touche à partir d'un caractère AZERTY"""
        key = cls.AZERTY_MAP.get(char)
        if key:
            cls.release_key(key)
            return True
